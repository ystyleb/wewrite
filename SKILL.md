...
- 相同 → 静默通过
- 不同 → 提示用户："WeWrite 有新版本可用（当前 X → 最新 Y），说「更新」即可升级。"**不阻断流程**，继续 1d
...
读取: {client_dir}/playbook.md（如果存在，按 confidence 分级执行）
读取: {client_dir}/history.yaml（最近 3 篇的 dimensions + closing_type 字段）
