...
Usage:
    ...
    python3 learn_edits.py --client winson --from-wechat  # auto-sync from WeChat draft box
    python3 learn_edits.py --client winson --summarize    # all lessons with confidence
    python3 learn_edits.py --client winson --summarize --json  # JSON output for agent
"""
...
def fetch_wechat_draft(client_ctx=None) -> tuple[str, str, str]:
    """
    ...
    """
    client_dir = client_ctx.dir if client_ctx else SKILL_DIR
    # Load config
    ...
    output_dir = SKILL_DIR / "output" / client_ctx.name if client_ctx and not client_ctx.is_legacy else SKILL_DIR / "output"
    draft_path = None
...
def save_lesson(diff_result: dict, draft_path: str, final_path: str, lessons_dir: Path = None) -> Path:
...
def load_all_lessons(lessons_dir: Path = None) -> list[dict]:
...
def summarize_lessons(as_json: bool = False, lessons_dir: Path = None):
...
def main():
    ...
    try:
        client_ctx = resolve_client_context(args.client, purpose="learn edits")
    ...
    if args.summarize:
        summarize_lessons(as_json=args.json, lessons_dir=lessons_dir)
        return

    if args.from_wechat:
        local_text, wechat_text, draft_path = fetch_wechat_draft(client_ctx=client_ctx)
        if local_text == wechat_text:
            print("\n微信草稿与本地文件内容一致，没有修改。")
            return
        diff_result = compute_diff(local_text, wechat_text)
        # Save with special marker for wechat source
        lesson_file = save_lesson(diff_result, draft_path, f"wechat:{draft_path}", lessons_dir=lessons_dir)
    ...
    # Save lesson
    lesson_file = save_lesson(diff_result, args.draft, args.final, lessons_dir=lessons_dir)
    print(f"\nLesson saved to: {lesson_file}")
    ...
    lesson_count = len(load_all_lessons(lessons_dir))
    ...
    if lesson_count >= 5 and lesson_count % 5 == 0:
        ...
        print(f"  python3 scripts/learn_edits.py --client {client_ctx.name} --summarize --json")
        print(f"Then update {playbook_path} with high-confidence patterns.")
